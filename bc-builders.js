...
//	oil_targets.forEach( function(e,i) { debugMsg("#"+i+" "+e.id+" "+e.name+" "+e.type+" "+e.player+" "+e.x+"x"+e.y); } );
	...
	if(buildersMainLen != 0){enumGroup(buildersMain).forEach( function(obj, iter){
		...
		if(factory_ready.length == 0 && research_lab_ready.length == 0){
			if ( rnd == 1 ) builderBuild(obj, "A0ResearchFacility", rotation);
			else builderBuild(obj, "A0LightFactory", rotation);
			return; //конец итерации, сл. строитель, типа continue в цикле for()
		}
		else if(factory_ready.length == 0) { builderBuild(obj, "A0LightFactory", rotation); return; }
		...
		else { if(oilHunt(obj, true)) return; }
		
		//Если строители свободны, ищем чего-бы достроить или починить
		var myBase = enumStruct(me);
		myBase.forEach( function(e){
			if(getDistance(e) > base_range)return;
			if(e.status == BEING_BUILT) {orderDroidObj(obj, DORDER_HELPBUILD, e); return;}
			if(e.health < 100) {orderDroidObj(obj, DORDER_REPAIR, e); return;}
			
		});
		
		//Если свободны, и далеко от базы - отправляем домой
		if(getDistance(obj) > 10) orderDroidLoc(obj,DORDER_MOVE,p_start.x,p_start.y);
...
function rigDefence(obj){
	...
	if(defQueue.length == 0) return false;
	if(defence.length == 0) return false;
	...
	var pos = pickStructLocation(obj,toBuild,defQueue[0].x, defQueue[0].y);
		...
		defQueue = myFarRigs.filter(
			function(e){
				...
				for (var i in myDefence){
					if (distBetweenTwoPoints(e.x,e.y,myDefence[i].x,myDefence[i].y) < 5) return false; //Если к качалке есть близко защита, пропускаем
				}
...
function oilHunt(obj, nearbase){
	...
	if(oil_targets.length == 0) { debugMsg("oilHunt(): Цели закончились"); return false; }
	if (builderBusy(obj)) return false;
	//Если строитель рядом с ресурсом
	for(var i in oil_targets){if(distBetweenTwoPoints(oil_targets[i].x,oil_targets[i].y,obj.x,obj.y) <= 10){
		...
		else if(oil_targets[i].type == STRUCTURE && !allianceExistsBetween(me, oil_targets[i].player)) {
			debugMsg("oilHunt(): Захват ресурса неудачен, уже занято");
			oil_targets.splice(i,1);
			continue;
		}else{
...
//			debugMsg("oilHunt: before splice "+oil_targets.length);
			oil_targets.splice(i,1);
//			debugMsg("oilHunt: after splice "+oil_targets.length);
			return true;
		}
	}}
//	debugMsg("oilHunt: before sort "+oil_targets.length);
	oil_targets = sortByDistance(oil_targets,obj,0,true);
//	debugMsg("oilHunt: after sort "+oil_targets.length);
	//orderDroidBuild(obj,DORDER_BUILD,"A0ResourceExtractor",oil_targets[0].x,oil_targets[0].y);
	...
	if(oil_targets.length > 1) oil_targets = oil_targets.slice(1);
