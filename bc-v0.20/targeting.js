function targetVTOL(){
	...
	var ready = enumGroup(VTOLAttacker).filter(function(e){if(e.action == 32 || e.action == 33 || e.action == 34 || e.action == 35 || e.action == 36 || e.action == 37 || e.action == 41 || e.action == 1)return false;return true});
	var group = enumGroup(VTOLAttacker).filter(function(e){debugMsg(e.id+"-"+e.action, 'vtol');if(e.action == 41 || e.action == 1)return true;return false});
...
//		group.forEach(function(e){var attack = orderDroidLoc(e, 40, target[0].x, target[0].y); debugMsg("Attacking: "+target[0].name+"-"+attack, 'vtol');}); // 40 - DORDER_CIRCLE
	...
	ready.forEach(function(e){orderDroidLoc(e, 40, base.x, base.y);});
}
